...
Function Get-CRC16 {
    [CmdletBinding()]
    param($HexData,$ByteData)

    if (($ByteData) -and ($HexData)) {
      Write-Verbose "I can't make up my mind, both parameters are set."
      Write-Verbose "I choose....HexData!"
      $bytes = $HexData
    }
    else {
      if (!$ByteData) {
        if (!$HexData) {
          Throw "Array of Hex or Byte values are needed to compute CRC"
        }
        else {
